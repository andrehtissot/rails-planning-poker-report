module Calculator
  ...
  module ClassMethods
    def return_mean values_array
      values_array.reduce(:+).try(:to_f).try(:/,values_array.size)
    end

    def return_median values_array
    ...
    def return_pred25 values_array
      (100/(values_array.length)) * (values_array.collect {|value| value.to_f <= 0.25 ? 1 : 0}).sum
    end

    def return_relative_diff_verbose relative_estimate_one, relative_estimate_two
      (self.return_relative_diff(relative_estimate_one, relative_estimate_two)*100).round(2)
    end

    def return_relative_diff relative_estimate_one, relative_estimate_two
